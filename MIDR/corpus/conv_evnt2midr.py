...
def notes2std(notes):
    ...
    std_notes = []
    for note in notes:
        std_note = StdNote.from_midi_note(note)
...
def notes2sna(notes):
    ...
    sna_notes = []
    for note in notes:
        sna_note = SnaNote.from_midi_note(note)
...
def evnt2midr(events, repr_type="standard"):
    """
    ...
    if repr_type == "standard":
        midrep_notes = notes2std(notes)
    
    elif repr_type == "sna":
        midrep_notes = notes2sna(notes)
